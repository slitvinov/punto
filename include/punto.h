...
void SetValues(struct Values val_d, struct Parametres par,
               struct Options opt, struct Values *val);
...
void OrderOfColumns(struct Options opt, int d, int ncol, int noptc,
                    int *posc, struct PosCol *p);
...
struct Punto *ReadBlock(int *n, struct DataFile, struct PosCol pos, long *,
                        struct Punto *p, int *);
struct Punto *ReadBlock_00(int *n, FILE * fp, struct PosCol pos,
                           int num_col, long *, struct Punto *p, int *);
...
void DrawTracePuntos(SDL_Surface * screen, struct Window w,
                     struct Universe u, struct Parametres par, float z,
                     int order);
int DrawPeriodicPuntos(SDL_Surface * screen, struct Window w,
                       struct Universe, struct Punto *, long *,
                       struct Point, float, struct Parametres val,
                       struct Values data);
