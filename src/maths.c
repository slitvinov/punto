...
void
TCR(float a, float b, float c, long n1, long n2,
    struct Point r, float comp, short field, int *reset_box,
    struct Punto *p)
{
    /* 
    ...
    for (i = n1; i < n2; i++) {
        p[i].x1 =
            (A1[0][0] * (p[i].x - r.x) + A1[0][1] * (p[i].y - r.y) +
             A1[0][2] * (p[i].z - r.z));
        p[i].y1 =
            (A1[1][0] * (p[i].x - r.x) + A1[1][1] * (p[i].y - r.y) +
             A1[1][2] * (p[i].z - r.z));
        p[i].z1 =
            (A1[2][0] * (p[i].x - r.x) + A1[2][1] * (p[i].y - r.y) +
             A1[2][2] * (p[i].z - r.z));
    }
    ...
    if (field == TRUE) {
        ...
        for (i = n1; i < n2; i++) {
            p[i].vx1 =
                ((A1[0][0] * (p[i].vx) + A1[0][1] * (p[i].vy) +
                  A1[0][2] * (p[i].vz))) / comp;
            p[i].vy1 =
                ((A1[1][0] * (p[i].vx) + A1[1][1] * (p[i].vy) +
                  A1[1][2] * (p[i].vz))) / comp;
            p[i].vz1 =
                ((A1[2][0] * (p[i].vx) + A1[2][1] * (p[i].vy) +
                  A1[2][2] * (p[i].vz))) / comp;
        }
    }
}                               /* -- funcion TCR3 */
...
void
Rapida(long *a, int izq, int drch, struct Punto *p)
     /* function Rapida:
