...
main(int argc, char *argv[], char **env)
{
    ...
    if (option.column == TRUE) {
        n_option_columns =
            GetColumns(param.colstr, dfile.ncol, pos_columns);
    }
    /* dimension */

    val_run.dimension =
        SetDimension(param, option, dfile.ncol, n_option_columns);
    ...
    if (option.type > 2 || option.type < 0) {   /* Default punto type */
        option.type = SPHERE;
        fprintf(stderr,
                "Warning: incorrect option, setting type to sphere\n");
    }
    ...
    if ((punto =
         (struct Punto *) calloc((size_t) max_N,
    ...
    dfile.nblocks = 0;
    if ((block =
         (struct Block *) calloc((size_t) 1,
                                 sizeof(struct Block))) == NULL) {
        perror("calloc");
        exit(EXIT_FAILURE);
    }
    block[0].read = FALSE;
    ...
    if (SDL_Init
        ((Uint32) (SDL_INIT_VIDEO | SDL_INIT_NOPARACHUTE | SDL_INIT_TIMER))
        < 0) {
    ...
    window.flags =
        (Uint32) (SDL_HWSURFACE | SDL_ANYFORMAT | SDL_RESIZABLE |
    ...
    screen =
        SDL_SetVideoMode(window.w, window.h, window.bpp, window.flags);
    if (!screen) {
    ...
    universe.escale.x =
        (float) window.w / (val_data.max_x - val_data.min_x);
    universe.escale.y =
        (float) window.h / (val_data.max_y - val_data.min_y);
    if (option.sizebox == TRUE) {
        universe.escale.x =
            (float) window.w / (param.box.x1 - param.box.x0);
        universe.escale.y =
            (float) window.h / (param.box.y1 - param.box.y0);
    }
    ...
    while (!done) {
            ...
            if (((option.anim == TRUE && kEOF == FALSE) || kp.space == TRUE || kp.pressed)) {   /* animation */
                ...
                do {
                    ...
                    if (time - time0 - delay + 10 > 0) {
                        ...
                        if (delay && option.pausa == TRUE
                            && option.anim == TRUE && option.fast == FALSE)
                            SDL_Delay(10);
                    }
                }
        ...
        /* mouse click handle */
        {
            ...
            if (md_x - filerect.x + 20 > 0
                ...
                if (md_y - filerect.y > 0
                    && (filerect.y + filerect.h) - md_y > 0) {
                    if (option.box == TRUE) {
                        inmouse = 1;
                        kEOF = FALSE;
                    }
...
void
MovePuntos(struct Universe u, struct Parametres par, struct Punto *p)
{
    ...
    if ((actualizar == TRUE || option.rotate == TRUE)) {
        ...
        if (option.mode == NAV)
            c = u.cv;
        TCR(u.ang.x, u.ang.y, u.ang.z, n1, n2, c, 1, option.field,
            &reset_box, p);
...
void
DrawAll(SDL_Surface * screen, struct Window win, struct Universe u,
        struct Parametres par, Uint32 background, struct DataFile df)
{
    ...
    if (actualizar == TRUE) {
        ...
        if (option.fast == FALSE) {
            //      printf("%f %f\n",u.escale.x,u.zoom);
            DrawPuntos(screen, win, u, punto, tabla, u.cw,
            ...
            if (option.trace) {
                DrawTracePuntos(screen, win, u, par, u.escale.x * u.zoom,
                                1);
            }
...
void
DrawPuntos(SDL_Surface * screen, struct Window w, struct Universe u,
    ...
    for (i = 0; i < numactivados; i++) {
        ...
        if (option.field == FALSE) {
            if (bola[idx].on == FALSE) {
                ...
                case SQUARE:
                    bola[idx].surface =
                        CreateRectangle(screen, fgcolor, fgcolor,
                                        2 * sizeidx + 1);
                    break;
                case SPHERE:
                    bola[idx].surface =
                        CreateBall3D(screen, fgcolor, sizeidx);
                    break;
                default:
                    bola[idx].surface =
            ...
            if (norm == 0)
                norm = 1;
            DrawArrow(screen, position, f, factor / norm, fgcolor,
                      kp.arrow);
...
void
DrawTracePuntos(SDL_Surface * screen, struct Window w, struct Universe u,
                struct Parametres par, float z, int order)
{
    ...
    if (actualizar == TRUE || option.rotate == TRUE) {
        for (i = 0; i < trace_cont; i++) {
            TCR(0, 0, 0, n1, n2, u.cm, 1, option.field, &reset_box,
                traceball[i]);
        }
    }
    for (i = n1; i < n2; i++) {
            ...
            if (option.trace == 1)
                ...
                if (j < trace_cont - 1) {
                    ...
                    if (fabs(traceball[l][i].x - traceball[k][i].x) < lenx) {
                        if (fabs(traceball[l][i].y - traceball[k][i].y) <
                            leny) {
                            DrawLine(screen,
                                     (int) (traceball[l][i].x1 * z +
                                            u.cw.x),
                                     (int) (w.h -
                                            (traceball[l][i].y1 * z +
                                             u.cw.y)),
                                     (int) (traceball[k][i].x1 * z +
                                            u.cw.x),
                                     (int) (w.h -
                                            (traceball[k][i].y1 * z +
                                             u.cw.y)), ledcolor);
...
int
DrawPeriodicPuntos(SDL_Surface * screen, struct Window w,
                   struct Universe u, struct Punto *p, long *t,
                   struct Point cw, float z, struct Parametres par,
                   struct Values data)
{
    int i, j, k, l;
    struct Punto *p0;
    ...
    c = u.cm;
    if ((p0 =
         (struct Punto *) malloc((size_t) numactivados *
                                 sizeof(struct Punto))) == NULL) {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
...
void
RangeOfValues(struct Punto *p, struct Options opt, struct Values *data)
{
    /* 
...
void
InfoSprites(struct Sprite *b, struct Options opt)
{
    int i;
    long mem;
    long cont;
    ...
    if (mem) {
        ...
        if ((((float) (maxpos - minpos) / mem > 2
              && maxpos - minpos > 16000000) || (cont > numactivados * 2
                                                 && cont > 256))
            && lastpurge > 16) {
            PurgeSprites(bola, 1);
            if (option.verbose) {
...
void
PurgeSprites(struct Sprite *b, int type)
{
    int i;
    ...
    case 0:
        for (i = 0; i < numsprites; i++) {
            ...
            if (b[i].on == TRUE
                ...
                b[i].surface = NULL;
                if (option.verbose > 1)
                    printf(" (%d)<%d+%d=%d ", i, i % max_nsize,
                           (int) (i / max_nsize), i);
                //  total_memory-=(i%max_nsize)*(i%max_nsize)*16;
                total_memory -=
                    ((float) i / MAX_NSCOLORS) * ((float) i /
                                                  MAX_NSCOLORS) * 16;
                cont1++;
            } else {
                cont2++;
...
void
Reescal_Color(int N, struct Punto *p1, struct Punto *p, float max_nc,
...
struct Punto *
ReadBlock(int *next, struct DataFile df, struct PosCol pos, long *numact,
          struct Punto *p, int *status)
{
    /*
    ...
    *status = TRUE;
    ...
    if ((temp =
         (double *) malloc((size_t) (df.ncol + 1) * sizeof(double))) ==
        NULL) {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
...
int
Arguments(int argc, char *argv[], struct Options *opt,
        ...
        if (*argv[i] == '-') {
            ...
            case 'z':
                if (strncmp("z", arg, MAX_WORD_LEN) == 0) { // choose columns
                    ...
                    strncpy(&par->colstr[MAX_WORD_LEN - 1], "\0",
                            (size_t) 1);
                }
                break;
            case 't':
            ...
            case 'B':
                if (strncmp("B", arg, MAX_WORD_LEN) == 0) { // show a border box
                    ...
                    if (pointer) {
                        ...
                        strncpy(&par->boxstr[MAX_WORD_LEN - 1], "\0",
                                (size_t) 1);
                        opt->sizebox = TRUE;
                        i++;
                    }
            ...
            case 'b':
                if (strncmp("bg", arg, MAX_WORD_LEN) == 0) {    // background color
                    ...
                    strncpy(&par->backname[MAX_WORD_LEN - 1], "\0",
                            (size_t) 1);
                }
                break;
            case 'f':
                if (strncmp("fg", arg, MAX_WORD_LEN) == 0) {    // foreground color
                    ...
                    strncpy(&par->forename[MAX_WORD_LEN - 1], "\0",
                            (size_t) 1);
                }
                break;
            case 'l':
                if (strncmp("lc", arg, MAX_WORD_LEN) == 0) {    // led color
                    ...
                    strncpy(&par->ledname[MAX_WORD_LEN - 1], "\0",
                            (size_t) 1);
                }
                break;
            case 'F':
                if (strncmp("F", arg, MAX_WORD_LEN) == 0) { // root file name to save pixmaps
                    ...
                    strncpy(&par->rootfilename[MAX_WORD_LEN - 1], "\0",
                            (size_t) 1);
                }
                break;
            case 'T':
                if (strncmp("T", arg, MAX_WORD_LEN) == 0) {
                    ...
                    strncpy(&par->titlewindow[MAX_WORD_LEN - 1], "\0",
                            (size_t) 1);
                }
                break;
            case 'P':
...
void
SetValues(struct Values val_d, struct Parametres par, struct Options opt,
          struct Values *val)
{
    /* 
...
int
SetDimension(struct Parametres par, struct Options opt, int ncol,
             int noptcol)
{
    /* 
    ...
    if (opt.dimension)
        ...
        if (opt.field) {
            ...
            if (ncol - num_options >= 6)
                ...
                fprintf(stderr,
                        "\twith option V must be 2,4 or 6 columns (sorry)\n");
                exit(EXIT_FAILURE);
            }
        }
    }
...
int
GetColumns(char *str, int ncolf, int *p_col)
{
    /*
       return:
    ...
    for (i = 0; i < 7; i++) {
        ...
        if (cpointer) {
            ...
            if (p_col[i] == 0) {
                fprintf(stderr,
                        "ERROR: invalid column number in -z option\n");
                exit(EXIT_FAILURE);
            }
            cpointer++;
    ...
    for (i = 0; i < n; i++) {
        if (p_col[i] > ncolf || p_col[i] < 0) {
            ...
            if (p_col[i] > ncolf) {
                fprintf(stderr,
                        "\tOnly there are %d columns in the first data block\n",
                        ncolf);
            }
            exit(EXIT_FAILURE);
        }
...
void
OrderOfColumns(struct Options opt, int d, int ncol, int noptc, int *posc,
               struct PosCol *p)
{
    /*
...
int
GetBox(int d, struct Parametres *par)
{
    /*
    ...
    if (cont != 2 * d) {
        fprintf(stderr,
                "Warning: Bad former argument -B. Must have %d elements \n",
                2 * d);
...
void
BoxProperties(struct Values val_d, struct Parametres par,
...
void
SetInitialValues(struct Options *opt, struct Keys *k)
{
    /*
