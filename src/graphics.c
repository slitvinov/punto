...
int
Led1(SDL_Surface * screen, Uint32 color, int x, int y, int n)
{
    /*    
    ...
    unsigned int mask[10] =
        { 126, 66, 55, 103, 75, 109, 125, 70, 127, 79 };
    int num[10];
    int temp;
    int size = FONTSIZE;
...
void
ReadRGBColors(char *fname, struct RGBColor *color, int n)
{
    /*
...
void
DrawArrow(SDL_Surface * screen, SDL_Rect pos, struct Point f, float factor,
          Uint32 color, int type)
{
    ...
    switch (type) {
    case 0:
        PutPixel(screen, (int) ((x + x1) * .5), (int) ((y + y1) * .5),
                 color);
        break;
    case 1:
    ...
    case 2:
        ...
        DrawLine(screen, (int) x1, (int) y1, (int) (x1 + x3),
                 (int) (y1 + y3), color);
...
void
DrawFile(SDL_Surface * screen, Uint32 color, Uint32 border,
         struct Rectangle rec, struct DataFile df)
{
    ...
    DrawRectangle(screen, rec.x, rec.y, rec.x + rec.l, rec.y + rec.h,
                  border);
    for (i = 1; i < (rec.h) && len > 0; i++) {
        DrawLine(screen, rec.x + 1, rec.y + i, rec.x + len, rec.y + i,
                 color);
    }
...
void
DrawScale(SDL_Surface * screen, Uint32 * color, Uint32 bordercolor,
...
void
GenColorTable(SDL_Surface * screen, Uint32 * colortable, int type)
{
    /*
    ...
    case RED:
        ...
        GenMiniColorTable(screen, colortable, MAX_NSCOLORS, color1,
                          color2);
        break;
    case GREEN:
    ...
    default:
        {
            ...
            GenMiniColorTable(screen, &colortable[cont], fin, color1,
                              color2);
...
void
GenMiniColorTable(SDL_Surface * screen, Uint32 * ct, int n,
...
void
WriteBitmap(SDL_Surface * bitmap, char *fpname)
{
/*
...
Uint32 color;
{
    ...
    if (dim <= 2) {
        DrawLine(w.screen,
                 (int) p[3].x, (int) p[3].y, (int) p[2].x, (int) p[2].y,
                 color);
        DrawLine(w.screen, (int) p[2].x, (int) p[2].y, (int) p[6].x,
                 (int) p[6].y, color);
        DrawLine(w.screen, (int) p[6].x, (int) p[6].y, (int) p[7].x,
                 (int) p[7].y, color);
        DrawLine(w.screen, (int) p[7].x, (int) p[7].y, (int) p[3].x,
                 (int) p[3].y, color);
    }
    if (dim == 3) {
        for (i = 0; i < 3; i++) {
...
void
InfoColors(struct RGBColor *rgbcolortable)
{
    int i;
    ...
    for (i = 0; i < MAX_NCOLORS; i++) {
        printf("%d: %d %d %d %s\n",
               i, (int) rgbcolortable[i].r, (int) rgbcolortable[i].g,
               (int) rgbcolortable[i].b, rgbcolortable[i].name);
    }
}
