...
/*    }  */
  ...
  if(d_x>0)incx=1;
  else if(d_x==0)incx=0;
  else incx=-1;
  
  if(d_y>0)incy=1;
  else if(d_y==0)incy=0;
  else incy=-1;
  
  d_x=abs(d_x);
  d_y=abs(d_y);
  if(d_x>d_y)distancia=d_x;
  else distancia=d_y;
...
void DrawCircle(SDL_Surface *screen, int x0, int y0, int r,Uint32 color){
  ...
  x=y;
  if(y)PutPixelCircle(screen,x,y,x0,y0,razon,color);
}

void FillRing(SDL_Surface *surface, int x0, int y0, int r,int h,Uint32 color){
  /*
...
SDL_Surface *CreateBall(SDL_Surface *screen,Uint32 color,Uint32 bcolor, int radio)
{
  ...
  temp=SDL_CreateRGBSurface(SDL_HWSURFACE,2*radio+1,2*radio+1,(int)screen->format->BitsPerPixel,0,0,0,0);
...
SDL_Surface * CreateBall3D(SDL_Surface *screen,Uint32 color, int radio)
{
  ...
  if(radio<2){
    ...
    for(i=(float)radio;i>0;i-=inci){
      ...
      b=blue-ib*i;
      color = SDL_MapRGB(screen->format, r,g,b);
      FillRing(temp,radio ,radio ,(int)(i),(int)(inci*1.4142+.5)+1, color); 
    }
  }
...
*/
  ...
  temp=SDL_CreateRGBSurface(SDL_SWSURFACE,radio,radio,(int)screen->format->BitsPerPixel,0,0,0,0);
