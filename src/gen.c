...
int main(){
  ...
  if (aleatorio){
	...
	fprintf(fp,"%f %f %f %f %d\n",
		(float)(rand()%1000)/100,(float)(rand()%1000)/100,(float)(rand()%1000)/100,
  ...
  if (fuente){
    ...
    int nummax=2000;
    x0=y0=z0=0;
   if(num>N)exit(EXIT_FAILURE);
    for(i=0;i<num;i++){vx[i]=vy[i]=vz[i]=x[i]=y[i]=z[i]=0;}
    strcpy(name,DATADIR);
  ...
  if (fuente2){
    ...
    int num=5000;
    if(num>N)exit(EXIT_FAILURE);
    for(i=0;i<num;i++){vx[i]=vy[i]=x[i]=y[i]=0;}
    strcpy(name,DATADIR);
